(() => {
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (rangeItems.length) rangeItems.forEach(((rangeItem, i) => {
                const fromValue = rangeItem.querySelector("[data-range-from]");
                const toValue = rangeItem.querySelector("[data-range-to]");
                const itemPrice = rangeItem.querySelector("[data-price-range-item]");
                const items = rangeItem.querySelector("[data-range-item]");
                if (itemPrice) {
                    nouislider.create(itemPrice, {
                        connect: true,
                    ...
                    itemPrice.noUiSlider.on("update", (function(values, handle) {
                        var value = values[handle];
                        if (handle) toValue.value = value; else fromValue.value = value;
                    }));
                    toValue.addEventListener("change", (function() {
                        itemPrice.noUiSlider.set([ null, this.value ]);
                    }));
                    fromValue.addEventListener("change", (function() {
                        itemPrice.noUiSlider.set([ this.value, null ]);
                    }));
                }
                if (items) {
                    nouislider.create(items, {
                        connect: true,
                        start: [ Number(fromValue.value), Number(toValue.value) ],
                        step: 1,
                        range: {
                    ...
                    items.noUiSlider.on("update", (function(values, handle) {
                        var value = values[handle];
                        if (handle) toValue.value = value; else fromValue.value = value;
                    }));
                    toValue.addEventListener("change", (function() {
                        items.noUiSlider.set([ null, this.value ]);
                    }));
                    fromValue.addEventListener("change", (function() {
                        items.noUiSlider.set([ this.value, null ]);
                    }));
                }
            }));
        }
                ...
                if (document.querySelector(".body-hero__slider")) new core(".body-hero__slider", {
                    ...
                    on: {
                        init: function(swiper) {
                            const allSlides = document.querySelector(".hero-fraction__all");
                            const allSlidesItems = document.querySelectorAll(".body-hero__slide:not(.swiper-slide-duplicate)");
                            allSlides.innerHTML = allSlidesItems.length;
                        },
                        slideChange: function(swiper) {
                            const currentSlide = document.querySelector(".hero-fraction__current");
                            currentSlide.innerHTML = swiper.realIndex + 1 < 10 ? `0${swiper.realIndex + 1}` : swiper.activeIndex + 1;
                        }
                    }
                });
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (document.querySelectorAll("[data-submenu]").length) if (!targetElement.matches(".catalog-header, .catalog-header *")) {
                const activeLink = document.querySelectorAll("._sub-menu-active");
                const activeBlock = document.querySelectorAll("._sub-menu-open");
                for (let i = 0; i < activeLink.length; i++) {
                    activeLink[i].classList.remove("_sub-menu-active");
                    activeBlock[i].classList.remove("_sub-menu-open");
                }
            }
            if (targetElement.closest("[data-parent]")) {
                ...
                const desktop = document.documentElement.clientWidth >= 991.98 ? true : null;
                console.log(desktop);
                if (subMenu && desktop) {
            ...
            if (targetElement.closest(".menu-catalog__item-back")) {
